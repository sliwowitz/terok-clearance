...
``terok hardening install``.  This module is the read-side:
...
def _loaded_profiles() -> dict[str, str]:
    ...
    Cached — both `loaded_confined_profiles` and `profile_modes` call
    this, and a sickbay row that surfaces clearance hardening will
    hit both.  Callers must treat the returned dict as read-only.
...
def install_command() -> str:
    ...
    for the privileged kernel-policy operations.  See
    `terok_clearance.hardening` for the install logic; this
    runtime-side module owns only the read-side probes.

    Single source for the command string so sickbay hints render
    the same invocation as setup tips and the rest of the CLI.
    """
