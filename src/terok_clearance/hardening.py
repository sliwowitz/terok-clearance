...
"""Clearance hardening — install + remove for the optional MAC layer.

Self-contained mirror of `terok_sandbox.hardening` for the
clearance hub + notifier domains.  Read that module's header for
the architectural rationale (one entry point per package, sudo only
where genuinely needed, drop-ins + restart as the calling user).

The duplication between this module and the sandbox one is by
design: terok-clearance is a leaf package (cannot import from
terok-sandbox per the AGENTS.md package boundary), and the install
sequences are short enough that one self-contained function per
package is more legible than a shared helper layer would be.
"""
...
"""Single source for the user-facing install invocation, shared
across all packages so renaming the orchestrator is one edit."""
