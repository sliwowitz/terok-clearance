...
The ``terok-dbus notify`` subcommand must send a real notification
...
class TestCliIntegration:
    ...
    def test_notify_prints_positive_id(self, dbusmock_session, notification_daemon):
        ...
        result = subprocess.run(
            [sys.executable, "-m", "terok_dbus._cli", "notify", "CLI test", "Body text"],
            capture_output=True,
            text=True,
            timeout=10,
            env=os.environ,
    ...
    def test_timeout_flag_accepted(self, dbusmock_session, notification_daemon):
        ...
        result = subprocess.run(
            [sys.executable, "-m", "terok_dbus._cli", "notify", "-t", "1000", "Timeout test"],
            capture_output=True,
            text=True,
            timeout=10,
            env=os.environ,
    ...
    def test_body_is_optional(self, dbusmock_session, notification_daemon):
        ...
        result = subprocess.run(
            [sys.executable, "-m", "terok_dbus._cli", "notify", "Summary only"],
            capture_output=True,
            text=True,
            timeout=10,
            env=os.environ,
